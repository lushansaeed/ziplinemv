...
function renderTemplate(values: Record<string, string>) {
  return bookingConfirmationTemplate.replace(/\{\{(\w+)\}\}/g, (_match, key) => values[key] ?? "");
}
...
export async function sendBookingConfirmationEmail(bookingId: string) {
  ...
  const addonsSummary = booking.addOns.length > 0
    ...
    : "None";
  const subject = `Your Zipline Booking is Confirmed - ${booking.reference}`;
