...
export async function ensureDayEndReportingSchema() {
  if (ensured) return;
  ...
  `);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "payments_collected_currency_method_status_created_at_idx" ON "payments" ("collected_currency", "method", "status", "created_at");`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "payments_received_by_user_id_created_at_idx" ON "payments" ("received_by_user_id", "created_at");`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "payment_method_changes_booking_id_created_at_idx" ON "payment_method_changes" ("booking_id", "created_at");`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "counter_floats_location_status_effective_date_idx" ON "counter_floats" ("location", "status", "effective_date");`);
  await prisma.$executeRawUnsafe(`CREATE UNIQUE INDEX IF NOT EXISTS "day_end_closings_report_date_location_cashier_id_key" ON "day_end_closings" ("report_date", "location", "cashier_id");`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "day_end_closings_report_date_location_idx" ON "day_end_closings" ("report_date", "location");`);
