...
const GUEST_MEDIA_EMAIL_TEMPLATE = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
        ...
        <table role="presentation" class="container" width="560" cellpadding="0" cellspacing="0" border="0" style="width:560px; max-width:560px; background:#FFFFFF; border-radius:12px; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,0.03);">
          ...
          <tr>
            <td class="px" style="padding:32px 36px 8px; text-align:center;">
              ...
              <div class="hero-title" style="font-size:30px; font-weight:500; color:#1A1A1A; letter-spacing:-0.02em; line-height:1.2;">What a day, {{first_name}}.</div>
            </td>
          </tr>
          <tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#F7F5EF; border-radius:12px;">
                <tr>
                  <td style="padding:26px 26px 24px; text-align:center;">
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center"><tr><td align="center" bgcolor="#D85A30" style="border-radius:8px; background:#D85A30;"><a href="{{drive_folder_url}}" target="_blank" class="cta-btn" style="display:inline-block; padding:14px 32px; font-family:'Trebuchet MS','Helvetica Neue',Arial,sans-serif; font-size:14px; color:#FFFFFF; text-decoration:none; font-weight:500; letter-spacing:0.03em; border-radius:8px;">Open your media folder &rarr;</a></td></tr></table>
...
function mediaFolderHtml(input: { customerName: string; folderUrl: string }) {
  return GUEST_MEDIA_EMAIL_TEMPLATE
    .replaceAll("{{first_name}}", escapeHtml(firstName(input.customerName)))
    .replaceAll("{{drive_folder_url}}", escapeHtml(input.folderUrl));
}
...
export async function sendMediaFolderEmail(bookingId: string, options: { force?: boolean; userId?: string } = {}) {
  ...
  const subject = `Your Zipline Maldives photos and videos | ${booking.reference}`;
