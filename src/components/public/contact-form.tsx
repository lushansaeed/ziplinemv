...
export function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-10 space-y-3">
        ...
        <p className="font-semibold site-heading">Message sent!</p>
        <p className="site-text-muted text-sm">We'll get back to you within 24 hours.</p>
        <button onClick={() => { setSent(false); setForm({ name: "", email: "", phone: "", message: "" }); }} className="text-xs site-text-muted mt-2 transition-colors">
  ...
  const inputCls = cn(
    "w-full rounded-xl px-4 py-3 text-sm bg-white border site-subtle-border",
    "site-body placeholder:text-slate-400",
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="text-xs site-text-muted font-medium">Name</label>
        ...
        <div className="space-y-1.5">
          <label className="text-xs site-text-muted font-medium">Email</label>
      ...
      <div className="space-y-1.5">
        <label className="text-xs site-text-muted font-medium">Phone (optional)</label>
        <input name="phone" value={form.phone} onChange={handleChange} placeholder="+960 7XX XXXX" className={inputCls} disabled={isPending} />
      </div>
      <div className="space-y-1.5">
        <label className="text-xs site-text-muted font-medium">Message</label>
      ...
      <button type="submit" disabled={isPending || !form.name || !form.email || !form.message} className={cn(
        ...
        "site-button shadow-brand-md hover:shadow-brand-lg",
