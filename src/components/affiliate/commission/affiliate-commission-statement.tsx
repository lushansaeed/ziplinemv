...
export function AffiliateCommissionStatement({ commissions, totals, commissionRate, commissionBasis }: any) {
  const getTotal = (status: string) => {
    const t = totals.find((x: any) => x.status === status);
    return Number(t?._sum.amount ?? 0);
  };

  const pending     = getTotal("PENDING");
  const paid        = getTotal("PAID");
  const totalEarned = totals.reduce((s: number, t: any) => s + Number(t._sum.amount ?? 0), 0);
  ...
  return (
    <div className="p-6 space-y-8">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Your rate"        value={`${commissionRate}%`}        icon={DollarSign}   iconColor="text-brand-ocean"
          subtitle={commissionBasis === "PACKAGE_ONLY" ? "on package price" : "on booking total"} />
      </div>
      ...
      <div className="admin-card bg-primary/5 border-primary/20 space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          {commissionBasis === "PACKAGE_ONLY"
            ...
            : "the full booking value including add-ons"
          }{" "}
          for every confirmed, paid booking attributed to your link or coupon.
        </p>
      </div>
      ...
      <div className="admin-card p-0 overflow-hidden">
        <table className="admin-table">
          <thead>
            <tr>
              <th>Booking</th><th>Customer</th><th>Date</th>
              <th>Package</th><th>Booking total</th><th>Rate</th>
              <th>Commission</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {commissions.length === 0 ? (
              ...
              commissions.map((c: any) => (
                <tr key={c.id} className="table-row-hover">
                  <td>
                    <a href={`/book/confirmation?ref=${c.booking.reference}`} target="_blank"
      ...
      {pending > 0 && (
        <div className="admin-card border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-900/10 space-y-2">
          <p className="font-semibold text-sm text-yellow-800 dark:text-yellow-400">
            {formatCurrency(pending)} pending payout
          </p>
          <p className="text-sm text-yellow-700 dark:text-yellow-500">
            Go to <a href="/affiliate/payouts" className="underline font-medium">Payouts</a> to request your earnings.
          </p>
        </div>
      )}
    </div>
